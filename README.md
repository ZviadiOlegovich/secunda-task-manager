...
### Индексы в MySQL для ускорения запросов

Индексы для оптимизации производительности вынесены в отдельную миграцию (`006_indexes.up.sql`). Создавать их заранее без реальных данных о нагрузке — преждевременная оптимизация: каждый индекс замедляет запись и занимает место. Оптимальный набор индексов определяется после профилирования реальных запросов через `EXPLAIN` и мониторинга slow query log.

### Поведение при удалении связанных данных (ON DELETE)

Все внешние ключи используют `RESTRICT` (поведение MySQL по умолчанию). Кандидаты на `ON DELETE CASCADE` — задача→история/комментарии, команда→участники/задачи. Для пользователей CASCADE нежелателен. Требует уточнения требований.
